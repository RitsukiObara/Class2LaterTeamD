...
namespace collision
{
	...
	// 障害物の当たり判定
	void ObstacleCollision(CPlayer& player, const float fWidth, const float fHeight, const float fDepth);	// 障害物の衝突判定
	...
	void BlockRectangleCollision(CBlock& block, CPlayer& player, const float fWidth, const float fHeight, const float fDepth);	// ブロックの矩形の当たり判定
